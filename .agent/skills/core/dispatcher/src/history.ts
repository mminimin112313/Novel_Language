...
export async function getSkillStats(toolName: string) {
    try {
        const data = await fs.readFile(HISTORY_PATH, 'utf8');
        const lines = data.split('\n').filter(l => l.trim());
        const entries = lines.map(l => JSON.parse(l) as LearningEntry);

        const relevant = entries.filter(e => e.tool === toolName);
        if (relevant.length === 0) return { successRate: 0.5, count: 0 }; // Neutral start
