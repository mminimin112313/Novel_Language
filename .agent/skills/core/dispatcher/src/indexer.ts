...
interface SkillEntry {
    ...
    description: string;
    path: string;
    category: string; // core, capabilities, workflows
}
...
async function buildIndex() {
    ...
    for (const file of files) {
        ...
        if (file.includes('/capabilities/')) category = 'capabilities';
        if (file.includes('/workflows/')) category = 'workflows';
        if (file.includes('/knowledge/')) category = 'knowledge';
